...
class RAGEvaluationEngine:
    ...
    def _load_sample_data(self):
        """샘플 데이터 로드 (테스트용)"""
        self.crawled_documents = {
            "git": "Git은 분산 버전 관리 시스템입니다. 주요 명령어: git init, git add, git commit, git push, git pull, git branch, git merge. Git의 장점은 오프라인 작업 가능, 빠른 속도, 강력한 브랜칭입니다.",
            "python": "Python은 고급 프로그래밍 언어입니다. 특징: 간단한 문법, 동적 타이핑, 풍부한 라이브러리, 크로스 플랫폼. 패키지 관리는 pip를 사용합니다. pip install package_name으로 설치. 가상환경은 python -m venv로 생성."
        }
    ...
    def save_results(self):
        ...
        print(f"  성능: {best['retrieval_score']:.4f}")
        print("="*70 + "\n")
...
def main():
    """메인 실행"""
    parser = argparse.ArgumentParser(description="RAG 평가 도구")
    parser.add_argument('--config', default='config.yaml', help='설정 파일 경로')
    parser.add_argument('--data-dir', default=None, help='크롤링 데이터 디렉토리')
    args = parser.parse_args()
    
    try:
        engine = RAGEvaluationEngine(args.config, args.data_dir)
        engine.evaluate()
        engine.save_results()
